...
##        self.loadDir()
    ...
    def loadDir(self, dbg = False):
        ...
        #print(self.imageDir)
        #print(self.category)
        self.imageList = glob.glob(os.path.join(self.imageDir, '*.tiff'))
...
#            self.egLabels[i].config(image = self.egList[-1], width = SIZE[0], height = SIZE[1])
    ...
    def loadImage(self):
        ...
        size = self.img.size
        self.factor = max(size[0]/(scrwidth-240), size[1]/(scrheight-120), 1.)
        ...
        if os.path.exists(self.labelfilename):
            with open(self.labelfilename) as f:
                for (i, line) in enumerate(f):
                    ...
                    tmp = line.split()
                    tmp[0], tmp[1], tmp[2], tmp[3] = int(tmp[0])/self.factor, int(tmp[1])/self.factor, int(tmp[2])/self.factor, int(tmp[3])/self.factor
                    print tmp
    ...
    def saveImage(self):
        with open(self.labelfilename, 'w') as f:
            ...
            for bbox in self.bboxList:
                print("box", bbox)
                f.write(' '.join(map(str, (int(int(bbox[0])*self.factor), int(int(bbox[1])*self.factor), int(int(bbox[2])*self.factor), int(int(bbox[3])*self.factor), bbox[4]))) + '\n')
